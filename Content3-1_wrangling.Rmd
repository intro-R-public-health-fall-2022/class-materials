...
Try, for example, the following code uses `read_csv` to read in a vector of mixed numeric and character type, while indicating that the data should be numeric (double). This results in a warning, and the `problems` function can be used to see why the warning is occurring:
```{r}
